...
public class ProjectTest {
    ...
    //this will test if the array list in hamper is not null when the client creates a new order
    ...
    //testFindCombinations() tests whether the method correctly returns combinations of food items. 
    @Test
    ...
    //testCalculateTotalExcess() tests the excess calories for a food group

    @Test
    public void testCalculateTotalExcess() 
    {
    ...
    //testCalculateShortage() tests any case where a group of food has a shortage
    @Test
    public void testCalculateShortage() 
    {
        Nutrition expectedNutrition = new Nutrition(1000, 25, 25, 25, 25);
        Food[] expectedFood = new Food[2];
        expectedFood[0] = new Food(10, "Burger", 25, 25, 25, 25, 100);
        expectedFood[1] = new Food(20, "Pizza", 30, 20, 25, 25, 500);
        int[] expectedShortageValue = {-75, -125, -100, -100};
        int[] actualShort = Inventory.calculateShortage(expectedFood, expectedNutrition);
        assertArrayEquals("The expected shortage of the inventory did not match the expected result, ", expectedShortageValue, actualShort);
    }

    //testCalculateTotalShortage() tests the total amount of shortage of food
    //which is why the expected is a neagtive number as that signfices the amount of 
    //food that is short for the method.

    @Test
    public void testCalculateTotalShortage() 
    {
        Nutrition expectedNutrition = new Nutrition(1000, 25, 25, 25, 25);
        Food[] expectedFood = new Food[2];
        ...
        assertEquals("The expected total shortage of the inventory did not match the expected result, ", expectedShortage, actualShortage);
    }

    //findBestCombination() tests whether the method  chooses the 
    //best combination from two options. 

    @Test
    public void testFindBestCombination()
    {
        Food[] goodCombination = new Food[2];
        Food apple = new Food(10, "Apple", 25, 25, 25, 25, 100);
        Food chocolate = new Food(14, "Chocolate", 25, 25, 25, 25, 1300);
        goodCombination[0] = apple;
        goodCombination[1] = chocolate;

        Food[] badCombination = new Food[1];
        badCombination[0] = apple;

        ArrayList<Food[]> combinations = new ArrayList<Food[]>();
        combinations.add(goodCombination);
        combinations.add(badCombination);

        Nutrition nutrition = new Nutrition(1400, 25, 25, 25, 25);
        Food[] actualCombination = Inventory.findBestCombination(combinations, nutrition);

        assertArrayEquals(goodCombination, actualCombination);

    }

    //testFindOrderCombo() tests whether the method  chooses the 
    //best combination from two options.
    
    @Test
    public void testFindOrderCombo()
    {
        ArrayList<Food> foodList = new ArrayList<Food>();
        Food apple = new Food(10, "Apple", 25, 25, 25, 25, 100);
        Food chocolate = new Food(14, "Chocolate", 25, 25, 25, 25, 1300);
        ...
        nutritionList[0] = new Nutrition(1400, 25, 25, 25, 25);
        Database database = new Database(null, foodList);
        Inventory.findOrderCombo(foodList, nutritionList);
